      ...
      <div class="grid mb-10 px-4 pt-6 grid-cols-1 dark:bg-gray-900">
        <div class="col-span-2">
          ...
          >
            ...
            >
              ...
              <div class="flex items-center mb-4 sm:mb-0">
                <form class="sm:pr-3" action="#" method="GET">
                  <label for="orders-search" class="sr-only">{{
              ...
              <div class="inline-block min-w-full align-middle">
                <div class="overflow-hidden shadow">
                  ...
                  >
                    ...
                    >
                      <tr
                        v-for="(closing, index) in closings.data"
                        :key="index"
                        class="hover:bg-gray-100 dark:hover:bg-gray-700"
                      >
                        <td class="tbl-column">{{ ++index }}</td>
                        <td class="tbl-column">{{ closing.date }}</td>
                        <td class="tbl-column">{{ priceFormat(closing.starting_cash)  }}</td>
                        <td class="tbl-column">{{ priceFormat(closing.cash_in) }}</td>
                        <td class="tbl-column">{{ priceFormat(closing.cash_out) }}</td>
                        <td class="tbl-column">{{ priceFormat(closing.total_cash) }}</td>
                        <td class="p-2 space-x-2 whitespace-nowrap">
                          <Link
                            :href="route('closing-day.show', closing.id)"
                            class="btn-primary"
                          >
                            <svg
                              class="w-4 h-4 mr-2"
            ...
            >
              <Pagination :links="props.closings" :filters="data.params" />
            </div>
          </div>
        </div>
      </div>

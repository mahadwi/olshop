...
import Show from "@/Pages/ReviewProduct/Show.vue";
import Delete from "@/Pages/ReviewProduct/Delete.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("review-product.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
                ...
                <div class="col-span-2">
                    ...
                    >
                                ...
                                <div class="overflow-hidden shadow">
                                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                        ...
                                        >
                                                ...
                                                <td class="tbl-column"> {{ review.product.name }}</td>
                                                <td class="tbl-column"> {{ review.user.name }}</td>
                                                <td class="tbl-column"> {{ review.date_review }}</td>
                                                <td class="tbl-column"> 
                                                    <Rating v-model="review.rating" readonly :cancel="false" />
                                                </td>
                                                <td class="tbl-column"> {{ review.review }}</td>
                                                <td class="tbl-column"> 
                                                    <button v-if="review.imageable.length > 0" @click="
                                                                (data.editOpen = true),
                                                                    (data.review = review)
                        ...
                        <div class="flex justify-between items-center p-2 border-t border-slate-200 dark:border-slate-700">
                            <Pagination :links="props.reviews" :filters="data.params" />
                        </div>
                    </div>
                </div>
            </div>

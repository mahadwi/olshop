...
import Edit from "@/Pages/Rekening/Edit.vue";
import Create from "@/Pages/Rekening/Create.vue";
import Delete from "@/Pages/Rekening/Delete.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("rekening.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const bankCode = props.bankCode.map((data) => ({
                ...
                <div class="col-span-2">
                    ...
                    >
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                        <thead class="bg-gray-100 dark:bg-gray-700">
                                            <tr>
                                                ...
                                                <th scope="col" class="tbl-head">
                                                    {{ lang().label.bank_account_holder }}
                                                </th>
                                                <th scope="col" class="tbl-head">
                                                    {{ lang().label.bank_account_number }}
                                                </th>
                                                <th scope="col" class="tbl-head">
                                                    {{ lang().label.logo }}
                                                </th>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                            <tr
                                            v-for="(rekening, index) in rekenings.data"
                                            :key="index"
                                            class="hover:bg-gray-100 dark:hover:bg-gray-700"
                                        >
                                                <td class="tbl-column pl-4"> {{ ++index }}</td>
                                                <td class="tbl-column"> {{ rekening.bank }}</td>
                                                <td class="tbl-column"> {{ rekening.bank_account_holder }}</td>
                                                <td class="tbl-column"> {{ rekening.bank_account_number }}</td>                                                
                                                <td class="w-32">
                                                    <img :src="rekening.logo_url" :alt="rekening.code">
                                                </td>
                        ...
                        <div class="flex justify-between items-center p-2 border-t border-slate-200 dark:border-slate-700">
                            <Pagination :links="props.rekenings" :filters="data.params" />
                        </div>
                    </div>
                </div>
            </div>

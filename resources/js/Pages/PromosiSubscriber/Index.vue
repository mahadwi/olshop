...
import Edit from "@/Pages/PromosiSubscriber/Edit.vue";
import Delete from "@/Pages/PromosiSubscriber/Delete.vue";
import Create from "@/Pages/PromosiSubscriber/Create.vue";
import Detail from "@/Pages/PromosiSubscriber/Detail.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("promo-subscribe.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
                ...
                <div class="col-span-2">
                    ...
                    >
                                ...
                                <div class="overflow-hidden shadow">
                                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                            <tr
                                                v-for="(subscribe, index) in promoSubscribers.data"
                                                :key="index"
                                                class="hover:bg-gray-100 dark:hover:bg-gray-700"
                                            >
                                                <td class="tbl-column pl-4"> {{ ++index }}</td>
                                                <td class="tbl-column"> {{ subscribe.title }}</td>
                                                <td class="tbl-column" v-html="truncate(subscribe.message, 25)">
                                                </td>
                                                <td class="tbl-column">
                                                    <button @click="
                                                                (data.detailOpen = true),
                                                                    (data.promoSubscribe = subscribe)
                        ...
                        <div class="flex justify-between items-center p-2 border-t border-slate-200 dark:border-slate-700">
                            <Pagination :links="props.promoSubscribers" :filters="data.params" />
                        </div>
                    </div>
                </div>
            </div>

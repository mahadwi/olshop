...
const update = () => {
  form.put(route("voucher.update", props.voucher?.id), {
    preserveScroll: true,
    onSuccess: () => {
      emit("close");
      form.reset();
...
watchEffect(() => {
  if (props.show) {
    ...
    form.date = props.voucher?.date;
    form.time = props.voucher?.time;
    form.capacity = props.voucher?.capacity;
    form.quota = props.voucher?.quota;
    form.use_for = props.voucher?.use_for;
    ...
    <Modal :show="props.show" @close="emit('close')">
      <form class="p-6" @submit.prevent="update">
        ...
        <div class="my-6 space-y-4">
          ...
          <div>
            <FwbInput
              :disabled="form.capacity != 'Limited'"
              v-model="form.quota"

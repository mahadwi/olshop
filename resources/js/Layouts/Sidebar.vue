<script setup>
  ...
  const activeSetting = route().current('about.*') || route().current('contact.*') || route().current('return-police.*') || route().current('privacy-police.*') || route().current('term-condition.*')  || route().current('delivery-shipping.*') || route().current('customer-care.*') || route().current('faq.*') ? '' : 'hidden'
